...
import { getAmmoDetailsForWeapon } from './logic/ammoLogic.js';
...
export default function CharacterScreen() {
  ...
  const [isEquipmentKitModalVisible, setIsEquipmentKitModalVisible] = useState(false);
  ...
  const remainingPerkPoints = isPerkAttributeMode 
    ? perkPointsToDistribute - (tempAttributes.reduce((sum, a) => sum + a.value, 0) - attributes.reduce((sum, a) => sum + a.value, 0))
    : 0;
  
  const remainingAttributePoints = isPerkAttributeMode ? remainingPerkPoints : remainingInitialPoints;
  ...
  const handleSavePerkAttributes = () => {
    if (remainingPerkPoints > 0) {
      Alert.alert("Ошибка", "Вы должны распределить все доступные очки атрибутов.");
      return;
    }
  ...
  const handleChangeSkillValue = (index, delta) => {
    ...
    setSkills(prev => {
      ...
      const isBonus = isGoodSoul && (trait?.modifiers?.goodSoulSelectedSkills || []).includes(skill.name);
  ...
  const handleSelectTrait = (traitName, newModifiersFromModal) => {
    ...
    if (newTrait.name === 'Одаренный') {
      ...
      chosenAttrs.forEach(attr => {
        setAttributes(prev => prev.map(a => a.name === attr ? { ...a, value: a.value + 1 } : a));
      });
  ...
  return (
    ...
    >
              ...
              onPress={() => {
                if (origin && origin.equipmentKits) {
                  if (equipment) {
                    ...
                    if (Platform.OS === 'web') {
                      if (window.confirm("Внимание! При выборе нового комплекта снаряжения весь текущий инвентарь будет сброшен. Продолжить?")) {
                  ...
                  Alert.alert("Информация", "Для данного происхождения нет комплектов снаряжения.");
                }
              }}
            />
      ...
      <AttributesSection
        ...
        onAttributeChange={isPerkAttributeMode ? handleTempAttributeChange : handleChangeAttribute}
